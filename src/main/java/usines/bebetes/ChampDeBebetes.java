...
import usines.bebetes.usine.*;
import usines.simu.Actionnable;
import usines.simu.Simulateur;
import usines.visu.Dessinable;
import usines.visu.Positionnable;
import usines.visu.VisualisateurAnime;
...
public class ChampDeBebetes extends VisualisateurAnime {
	...
	private BebeteFactory factory;

	public ChampDeBebetes(int largeur, int hauteur, int nb) {
		super(largeur,hauteur);
		setPreferredSize(new Dimension(largeur, hauteur));
		// Choix de la factory pour les bebetes
		this.factory = EcoSystemeFactory.getFactory("mixte");
	...
	public ArrayList<? extends Bebete> fabriqueBebetes(int nb) {
		...
		for (int i = 0; i < nb; i++) {
			...
			//nouvBebetes.add(new BebeteEmergente(this, x, y, direction, vitesse, new Color(r, g, b)));
			nouvBebetes.add(factory.getBebete(this, x, y, direction, vitesse, new Color(r, g, b), i));
		}
		return nouvBebetes;
	}
